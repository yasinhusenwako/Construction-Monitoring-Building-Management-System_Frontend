...
export function KeycloakAuthProvider({
  ...
  useEffect(() => {
    ...
    const initTimeout = setTimeout(() => {
      console.error(
        "Keycloak initialization timeout - proceeding without Keycloak",
      );
      setLoading(false);
    ...
    keycloak
      ...
      .then((auth) => {
        ...
        if (auth) {
          // Get user profile directly from token claims (avoid CORS issues)
          ...
          if (keycloak.token) {
            try {
              ...
              const extractedDivisionId =
                tokenPayload.divisionId ||
                tokenPayload.division_id ||
                tokenPayload.divisionid;

              if (extractedDivisionId) {
                setDivisionId(extractedDivisionId);
                console.log(
                  "✅ Division ID extracted from token:",
                  extractedDivisionId,
                );
              } else {
                // Fallback: Fetch divisionId from backend API
                console.log(
                  "⚠️ divisionId not in token, fetching from backend...",
                );
                fetch("http://localhost:8081/api/users/me", {
                  ...
                  .then((userData) => {
                    ...
                    if (userData.divisionId) {
                      ...
                      console.log(
                        "✅ Division ID fetched from backend:",
                        userData.divisionId,
                      );
                    } else {
                  ...
                  .catch((error) => {
                    console.error(
                      "❌ Failed to fetch divisionId from backend:",
                      error,
                    );
                    setDivisionId(undefined);
                  });
              ...
              console.error(
                "❌ Failed to extract divisionId from token:",
                error,
              );
              setDivisionId(undefined);
            }
                ...
                if (refreshed) {
                  ...
                  if (keycloak.token) {
                    try {
                      ...
                      console.error(
                        "Failed to extract divisionId from refreshed token:",
                        error,
                      );
                    }
                  }
          ...
          (window as any).keycloakRefreshCleanup = () =>
      ...
      .catch((error) => {
        ...
        if (
          ...
          console.error(
            "🚨 CSP is blocking Keycloak! Check Content-Security-Policy headers.",
          );
        }
