...
export function NewBookingPage() {
  ...
  const handleSubmit = async () => {
    ...
    const bookingItem: Booking = {
      ...
      purpose: isOfficeAllocation ? b1Form.reason : b2Form.purpose,
      requirements: isOfficeAllocation
        ? b1Form.specialReqs.join(", ")
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {spaces.map((space) => (
                <button
                  key={space.id}
                  onClick={() =>
                    !space.available ? undefined : updateB2("space", space.id)
                  }
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${space.available ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
                    >
                      {space.available
