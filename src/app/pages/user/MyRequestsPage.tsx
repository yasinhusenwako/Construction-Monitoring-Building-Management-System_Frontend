...
import { mockProjects, mockBookings, mockMaintenance } from "../../data/mockData";
import { StatusBadge } from "../../components/common/StatusBadge";
import { getUserFacingStatus, WorkflowRole } from "../../lib/workflow";
import { Search, Filter, ArrowRight, FolderOpen, Calendar, Wrench, ExternalLink } from "lucide-react";
...
export function MyRequestsPage() {
  ...
  const [filterType, setFilterType] = useState<"all" | "Project" | "Booking" | "Maintenance">("all");

  const allRequests = useMemo(() => {
    const myProjects = mockProjects.filter(p => p.requestedBy === uid).map(p => ({
      id: p.id,
      title: p.title,
    ...
    const myBookings = mockBookings.filter(b => b.requestedBy === uid).map(b => ({
      id: b.id,
    ...
    const myMaintenance = mockMaintenance.filter(m => m.requestedBy === uid).map(m => ({
      id: m.id,
      title: m.title,
    ...
    return [...myProjects, ...myBookings, ...myMaintenance].sort((a, b) => b.date.localeCompare(a.date));
  }, [uid]);
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case "Booking": return <Calendar size={16} className="text-purple-600" />;
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden text-sm">
        <div className="p-4 border-b border-border bg-gray-50/50 flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-2 overflow-x-auto">
            {(["all", "Project", "Booking", "Maintenance"] as const).map((type) => (
              <button
                key={type}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 text-muted-foreground font-medium uppercase text-[10px] tracking-wider">
                ...
                <th className="px-6 py-3 border-b border-border">Submitted On</th>
              ...
              {filteredRequests.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center text-muted-foreground">
                       ...
                       <p className="font-medium text-gray-500">No requests found matching your filters.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredRequests.map((req) => (
                  <tr key={req.id} className="hover:bg-gray-50/80 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <StatusBadge status={getUserFacingStatus(req.status, 'user')} />
                    </td>
