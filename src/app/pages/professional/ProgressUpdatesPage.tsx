  ...
  const handleUpdateStatus = (taskId: string, newStatus: WorkflowStatus) => {
    ...
    setTimeout(() => {
      setActiveTasks((prev) => {
        ...
        const updatedTask = next.find((t) => t.id === taskId);
        if (updatedTask) {
          if (taskId.startsWith("PRJ-")) updateProject(updatedTask as any);
          else if (taskId.startsWith("BKG-")) updateBooking(updatedTask as any);
          else updateMaintenance(updatedTask as any);
        }
        return next;
      });
      setUpdating(null);
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#0E2271]">
          {t("professional.progressUpdates")}
        </h1>
        ...
        {activeTasks.length === 0 ? (
          ...
          activeTasks.map((task) => {
            const allowed = getAllowedTransitions(
              "professional",
              task.status as WorkflowStatus,
            );

            return (
              <div
                key={task.id}
                className="bg-white rounded-2xl border border-border shadow-sm hover:shadow-md transition-all p-6 space-y-4"
              >
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      <div className="flex items-center gap-1">
                        <MapPin size={12} />{" "}
                        {(task as any).location || (task as any).space}
                      </div>
                      <div className="flex items-center gap-1">
                ...
                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {allowed.map((nextStatus) => (
                        ...
                        >
                          ...
                          {nextStatus === "In Progress"
                            ? t("professional.markInProgress")
                            : nextStatus === "Completed"
                              ? t("professional.markCompleted")
                              : `${t("professional.markAs")} ${t(`status.${nextStatus.toLowerCase().replace(/ /g, "")}`)}`}
                        </button>
                      ))}
                      {allowed.length === 0 && (
                        <p className="text-xs text-muted-foreground italic">
                          {t("professional.noActionsRequired")}
                        </p>
                      )}
                    </div>
                  </div>
                  <button
                    onClick={() => {
                      const path = task.id.startsWith("PRJ-")
                        ? `/dashboard/projects/${task.id}`
                        : task.id.startsWith("BKG-")
                          ? `/dashboard/bookings/${task.id}`
                          : `/dashboard/maintenance/${task.id}`;
                      router.push(path);
                    }}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1">
                    ...
                    {t("professional.addProgressNote")}
                  </label>
