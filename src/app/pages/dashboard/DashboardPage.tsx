...
export function DashboardPage() {
  ...
  const [maintenanceItems, setMaintenanceItems] = useState(mockMaintenance);
  ...
  const assignedMaintenance = maintenanceItems.filter(
    (m) => m.assignedTo === uid,
  );
  const supervisorIdPath = maintenanceItems.some((m) => "supervisorId" in m);
  const supervisorMaintenance = maintenanceItems.filter((m) => {
    if (supervisorIdPath) return (m as any).supervisorId === uid;
    return (
          ...
          {role === "professional" && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard
                ...
                value={assignedMaintenance.length}
              ...
              <StatCard
                ...
                value={
                  assignedMaintenance.filter((m) => m.status === "In Progress")
                    .length
                }
