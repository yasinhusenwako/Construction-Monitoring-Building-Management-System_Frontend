...
import { ProjectAssignment, ProfessionalReport, markReportsAsRead } from '@/lib/multi-professional-api';
import { Trash2, MessageSquare, ChevronDown, ChevronUp, Clock, User as UserIcon } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import { enUS } from 'date-fns/locale';

interface AdminAssignmentsListProps {
...
export function AdminAssignmentsList({
    ...
    return (
      <div className="bg-gray-50 border border-dashed border-gray-300 rounded-lg p-8 text-center">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Use the form above to assign your first professional
        </p>
      </div>
    );
  }
  ...
  const handleToggleReports = async (assignmentId: number) => {
    if (expandedAssignmentId === assignmentId) {
      ...
      if (unreadCount > 0) {
        try {
          await markReportsAsRead(assignmentId);
          onViewReports(assignmentId); // Trigger refresh in parent
        } catch (error) {
            ...
            <div className="p-4">
              <div className="flex items-start justify-between mb-3">
                ...
                <button
                  ...
                  title="Remove professional from project"
                >
                  <Trash2 size={14} />
                </button>
              </div>
