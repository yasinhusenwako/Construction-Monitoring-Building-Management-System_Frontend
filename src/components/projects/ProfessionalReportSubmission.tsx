...
interface ProfessionalReportSubmissionProps {
  assignment: ProjectAssignment;
  systemUsers: Array<{ id: string; name: string }>;
  onBack: () => void;
}

export function ProfessionalReportSubmission({
  assignment,
  systemUsers,
  onBack,
}: ProfessionalReportSubmissionProps) {
  ...
  const loadReports = async () => {
    ...
    try {
      const data = await getMyAssignmentReports(assignment.id);
      setReports(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await submitProfessionalReport(assignment.id, reportText);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <textarea
            ...
            disabled={loading}
            placeholder="e.g., 'Completed electrical wiring on floors 1-2. Installed 32 outlets. Material for floor 3 arrived. Ready to continue tomorrow.'"
            rows={5}
