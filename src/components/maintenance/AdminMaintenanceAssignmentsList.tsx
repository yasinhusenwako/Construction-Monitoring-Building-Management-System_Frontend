...
interface AdminMaintenanceAssignmentsListProps {
...
export function AdminMaintenanceAssignmentsList({
  assignments,
  systemUsers,
  allReports,
  onDeactivate,
  ...
  onApprove,
  onReject,
}: AdminMaintenanceAssignmentsListProps) {
  ...
  const handleToggleReports = async (assignmentId: number) => {
    if (expandedAssignmentId === assignmentId) {
      ...
      if (unreadCount > 0) {
        try {
          await markMaintenanceReportsAsRead(assignmentId);
  ...
  const handleDelete = async (assignmentId: number) => {
    if (!confirm('Are you sure you want to remove this professional from the maintenance request?')) {
      return;
    }
    setDeleting(assignmentId);
    try {
      await onDeactivate(assignmentId);
      toast.success('Professional removed from maintenance request');
    } catch (error) {
            ...
            <div className="p-4">
              <div className="flex items-start justify-between mb-3">
                ...
                <button
                  ...
                  title="Remove professional from maintenance"
                >
                  <Trash2 size={14} />
                </button>
              </div>
